...
class TestRegisterSuccess:
    ...
    def test_register_password_is_hashed_in_db(self, client, db_session, valid_register_payload):
    ...
    def test_register_stores_correct_username(self, client, db_session, valid_register_payload):
        client.post("/api/register", json=valid_register_payload)
        from database import User
        user = db_session.query(User).filter(User.username == "testuser").first()
        assert user.username == "testuser"

    def test_register_stores_timestamp(self, client, db_session, valid_register_payload):
        client.post("/api/register", json=valid_register_payload)
        from database import User
        user = db_session.query(User).filter(User.username == "testuser").first()
